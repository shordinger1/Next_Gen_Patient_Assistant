...
def main():
    ...
    # 1. 未量化模型推理和显存计算
    # print("\n====== 未量化模型推理 ======")
    tokenizer = AutoTokenizer.from_pretrained(args.unquantized_model_path, trust_remote_code=True, torch_dtype=torch.float16)

    # gpu_memory_before_unquantized = get_model_memory_usage(device)  # 模型加载前的显存
    # unquantized_model = AutoModelForCausalLM.from_pretrained(args.unquantized_model_path, trust_remote_code=True, torch_dtype=torch.float16)
